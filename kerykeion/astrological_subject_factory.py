...
def ephemeris_context(
    ...
    if config.zodiac_type == "Sidereal":
        ...
        # Explicitly reset sidereal mode for Tropical charts to avoid
        # cross-test contamination from previous Nakshatra calculations.
...
class AstrologicalSubjectFactory:
    ...
    def from_birth_data(
        ...
        *,
        ...
        with ephemeris_context(
            ...
            # -----------------------------------------------------------------
            # NAKSHATRA CALCULATION (Isolated Global State)
            # -----------------------------------------------------------------
            # We calculate this at the end of the main Tropical/Sidereal
            # calculations to ensure that calling swe.set_sid_mode() doesn't
            # affect the precision of the main chart positions.
            if config.zodiac_type == "Sidereal":
            ...
            # Update all calculated points with Nakshatra info
            nak_ayan_val = calc_data.get("nakshatra_ayanamsa_value")
            if nak_ayan_val is not None:
                for key, value in calc_data.items():
                    if isinstance(value, KerykeionPointModel):
                        nak_data = get_nakshatra_data(value.abs_pos, nak_ayan_val)
                        value.nakshatra = nak_data.get("nakshatra")
    ...
    def from_iso_utc_time(
        ...
        Args:
            ...
            nakshatra_ayanamsa (SiderealMode, optional): Sidereal mode to use for
                Nakshatra calculation. Defaults to 'LAHIRI'.
    ...
    def _calculate_arabic_part(
        ...
        data[part_key] = get_kerykeion_point_from_degree(
            part_deg, part_name, point_type=point_type
        )
            ...
            try:
                ...
                if should_calculate("Vertex"):
                    data["vertex"] = get_kerykeion_point_from_degree(
                        vertex_deg, "Vertex", point_type=point_type
                    )
